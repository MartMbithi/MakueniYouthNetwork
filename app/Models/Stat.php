...
final class Stat
{
    ...
    {
        $stmt = Database::connection()->query('SELECT * FROM stats ORDER BY sort_order, id');
    ...
    {
        $stmt = Database::connection()->prepare('SELECT * FROM stats WHERE id = :id');
        $stmt->execute([':id' => $id]);
        $row = $stmt->fetch();
    ...
    public static function create(array $data): int
    {
        $stmt = Database::connection()->prepare(
            'INSERT INTO stats (label, value, sort_order) VALUES (:label, :value, :sort_order)'
        );
        $stmt->execute([
            ':label'      => $data['label'],
            ':value'      => $data['value'],
    ...
    {
        $stmt = Database::connection()->prepare(
            'UPDATE stats SET label=:label, value=:value, sort_order=:sort_order WHERE id=:id'
        );
        $stmt->execute([
            ':label'      => $data['label'],
            ':value'      => $data['value'],
    ...
    {
        $stmt = Database::connection()->prepare('DELETE FROM stats WHERE id = :id');
        $stmt->execute([':id' => $id]);
    }
}
