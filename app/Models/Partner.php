...
final class Partner
{
    ...
    {
        $stmt = Database::connection()->query('SELECT * FROM partners ORDER BY sort_order, name');
    ...
    public static function create(array $data): int
    {
        $stmt = Database::connection()->prepare(
            'INSERT INTO partners (name, logo, url, sort_order)
             VALUES (:name, :logo, :url, :sort_order)'
        );
        $stmt->execute([
            ':name'       => $data['name'],
            ':logo'       => $data['logo'] ?? null,
    ...
    {
        $stmt = Database::connection()->prepare(
            'UPDATE partners SET name=:name, logo=:logo, url=:url, sort_order=:sort_order WHERE id=:id'
        );
        $stmt->execute([
            ':name'       => $data['name'],
            ':logo'       => $data['logo'] ?? null,
