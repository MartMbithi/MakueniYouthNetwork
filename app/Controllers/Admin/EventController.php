...
final class EventController
{
    ...
    {
        return View::render('admin/events/index.twig', [
            'title'  => 'Events',
            'events' => Event::all(),
        ]);
    }
    ...
    {
        return View::render('admin/events/form.twig', [
            'title'  => 'New event',
            'event'  => $this->blank(),
            'action' => '/admin/events',
            'mode'   => 'create',
    ...
    public function store(): string
    {
        ...
        View::flash('Event created.', 'success');
        Response::redirect('/admin/events/' . $id . '/edit');
        return '';
    }
    ...
    public function update(string $id): string
    {
        ...
        View::flash('Event saved.', 'success');
        Response::redirect('/admin/events/' . $id . '/edit');
        return '';
    }
    ...
    private function collect(): array
    {
        ...
        $status = $action === 'publish' ? 'published' : ($action === 'draft' ? 'draft' : 'draft');
        return [
            ...
            'ends_at'     => trim((string) Request::input('ends_at', '')),
            'status'      => $status,
    ...
    private function validate(array $data): array
    {
        ...
        if ($data['starts_at'] === '')  $errors['starts_at'][] = 'A start date/time is required.';
        return $errors;
    }
    ...
    private function resolveSlug(string $source, ?int $id): string
    {
        ...
        return Slug::unique($source, static function (string $slug) use ($pdo, $id): bool {
            $stmt = $pdo->prepare('SELECT id FROM events WHERE slug = :s LIMIT 1');
            $stmt->execute([':s' => $slug]);
            $row = $stmt->fetch();
    ...
    private function blank(): array
    {
        return [
            ...
            'cover_image' => null, 'venue' => '', 'starts_at' => '', 'ends_at' => null,
            'status' => 'draft',
        ];
    }
}
